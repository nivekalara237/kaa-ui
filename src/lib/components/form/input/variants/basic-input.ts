...
export class BasicInput extends AbstractInput {
  getInputClassNames(): string {
    ...
    builder.append(inputRoundedSizeMapping[this.options.fullRounded ? 'full' : this.options.size]);
    if (!this.options.floatingLabel) {
      ...
      builder.append(
        // `peer ${this.options.hasIconAt ? 'py-3 pe-4 ps-11' : 'p-3'} ` +
        "peer w-full px-4 transition-all rounded-lg border border-gray-200 " +
